...
Let $d^{k} \in \mathcal{D}(x_k)$, $x^{k} \in \mathcal{F}$. We denote by $\Delta^{k}$ the maximum feasible step along $d^{k}$. It is easy to see that:
\begin{equation*}\label{eq:maxmaxstep}
\Delta^k= \min \{ x^k_{j(k)}-l_{j(k)}, u_{i(k)}-x^k_{i(k)}\}
\end{equation*}
...
In order to find a feasible step along a descent direction in $x^k$, we use a line search method called quadratic line search. The QLS algorithm procedure (Algorithm \ref{alg:qls}) starts from $\alpha^k = \Delta^k$ and decreases $\alpha^k$ until:
\begin{equation}
f(x^k+\alpha^kd^k) \le  f(x^k)- \gamma (\alpha^k||d^k||)^2
\end{equation}
 where $d^k$ is a descent direction in $x^k$.
...
the structure of the feasible set and possibly the form of the objective function. The first block contains the constrained variables $x$, the second block contains
...
\begin{algorithm}[ht]
 ...
 \eIf{$(k \enskip \text{mod} \enskip M = 0)$ or $(\alpha^{k-1} = \Delta^{k-1})$}
  {
...
Instruction 4 of Algorithm \ref{alg:decMVP} contains the condition
\begin{equation}\label{eq:maxdelta}
\alpha^{k-1} = \Delta^{k-1}
\end{equation}
for convergence reasons, where $\Delta^{k-1}$ is the maximum feasible step at iteration $k-1$ as defined in (\ref{eq:maxmaxstep}). Obviously, condition (\ref{eq:maxdelta}) is not evaluated at the first iteration (i.e. for $k=0$).\\
...
\end{proposition}
\begin{proof}
The firts assertion is given by the Algorithm \ref{alg:proximal} statement which say:
\begin{equation}
...
In Algorithm \ref{alg:proximal} we choose a generic violating pair, without no assumptions on the \textquotedblleft quality " of this pair. In our implementation, we select a \textquotedblleft sufficient" violating pair instead, selecting the MVP among a subset of asset's indexes $\left\{1,.., n\right\}$. For each iteration $k$, let
