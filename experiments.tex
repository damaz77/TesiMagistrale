\section{Datasets}
In this section we briefly describe the datasets on which we tested some of the previously presented algorithms. The datasets used are the following:
\begin{itemize}
\item Random generated covariance matrices
\item S\&P-500 (476 assets)
\item NASDAQ (2196 assets)
\end{itemize}
\subsection{Random generated dataset}
For our purposes, we need to generate an $n \times n$ random semidefinite positive matrix, that represents the covariance matrix between the $n$ assets. Let $A$ be an $n \times n$ matrix with elements 
\begin{equation}
A_{i,j} \sim \mathbb{U}[0,1] \quad \forall i,j \in \{1,..,n\}
\end{equation}
where $\mathbb{U}[0,1]$ is the uniform distribution among the interval $[0,1]$. We can obtain the random semidefinite positive covariance matrix $\Sigma_{\text{RAND}}$ using
\begin{equation}
\Sigma_{\text{RAND}} = A \cdot A^T
\end{equation}
We can try to characterize this dataset highlighting some of his features:
\begin{itemize}
\item $\Sigma_{\text{RAND}i,j} \geq 0 \quad \forall i,j \in \{1,..,n\}$ 
\item $\mu(\Sigma_{\text{RAND}}) = \frac{n}{4}$
\end{itemize}

\subsection{S\&P-500 dataset}
This dataset is a $476 \times 476$ covariance matrix with:
\begin{itemize}
\item $\min \{\Sigma_{\text{S\&P}}\} = -2.5212 \times 10^{-4} $
\item $\max \{\Sigma_{\text{S\&P}}\} = 6.6626 \times 10^{-3} $
\item $\mu(\Sigma_{\text{S\&P}}) = 3.0559 \times 10^{-4} $
\item $\sigma^2(\Sigma_{\text{S\&P}}) = 5.4873 \times 10^{-8}$
\end{itemize}

\subsection{NASDAQ dataset}
This dataset is a $2196 \times 2196$ covariance matrix with:
\begin{itemize}
